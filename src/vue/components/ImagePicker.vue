    ...
    <Teleport to="body">
      ...
      >
        <label class="nexusdown-image-picker__field">
          <span>图片地址</span>
          <input v-model="srcValue" aria-label="图片地址" type="url" placeholder="https://" @keydown.enter.prevent="apply" />
        </label>
        <label class="nexusdown-image-picker__field">
          <span>替代文本</span>
          <input v-model="altValue" aria-label="图片描述" type="text" placeholder="图片描述" @keydown.enter.prevent="apply" />
        </label>
        <div class="nexusdown-image-picker__actions">
          ...
          <button type="button" aria-label="应用图片" :disabled="!srcValue.trim()" @click="apply">应用</button>
        </div>
      </div>
    </Teleport>
  </div>
